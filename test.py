...
categories = [settings.STAGE1_CATEGORIES, settings.STAGE2_CATEGORIES]
...
for i in range(len(stages)):
    ...
    data.clear()
    df = pd.read_excel(f'./stage{i+1}_data_template.xlsx', header=0, index_col=0)
    base_dir = f'./raw_data/{stages[i]}/'
    if f'stage{i+1}_data.xlsx' not in os.listdir('.'):
        print('Preparing Data')

        for category in  os.listdir(base_dir):
            path = os.path.join(base_dir, category) + '/'
            for file in os.listdir(path):
                data.append((os.path.join(path, file), categories[i].index(category)))
        data = data[:600]
        random.shuffle(data)
        ...
        df.to_excel(f'./stage{i+1}_data.xlsx')
        x_data = []
        y_data = []
